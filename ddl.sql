...
CREATE TABLE IF NOT EXISTS `mydb`.`Seller` (
  ...
  INDEX `fk_Seller_Visitor1_idx` (`Visitor_Visitor_id` ASC) VISIBLE,
  CONSTRAINT `fk_Seller_Visitor1`
...
CREATE TABLE IF NOT EXISTS `mydb`.`Ticket` (
  ...
  INDEX `fk_Ticket_Seller1_idx` (`Seller_Seller_id` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `mydb`.`Equipment_has_Stage`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`Equipment_has_Stage` ;

CREATE TABLE IF NOT EXISTS `mydb`.`Equipment_has_Stage` (
  `Equipment_Name` VARCHAR(45) NOT NULL,
  `Stage_Stage_id` INT NOT NULL,
  `Quantity` INT NOT NULL,
  PRIMARY KEY (`Equipment_Name`, `Stage_Stage_id`),
  INDEX `fk_Equipment_has_Stage_Stage1_idx` (`Stage_Stage_id` ASC) VISIBLE,
  INDEX `fk_Equipment_has_Stage_Equipment1_idx` (`Equipment_Name` ASC) VISIBLE,
  CONSTRAINT `fk_Equipment_has_Stage_Equipment1`
    FOREIGN KEY (`Equipment_Name`)
    REFERENCES `mydb`.`Equipment` (`Name`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Equipment_has_Stage_Stage1`
    FOREIGN KEY (`Stage_Stage_id`)
    REFERENCES `mydb`.`Stage` (`Stage_id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
...
-- -----------------------------------------------------
-- Table `mydb`.`Staff_has_Performance`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`Staff_has_Performance` ;

CREATE TABLE IF NOT EXISTS `mydb`.`Staff_has_Performance` (
  `Staff_Staff_id` INT NOT NULL COMMENT 'αυτο το table διαχειρίζεται το προσωπικό που εργάζεται σε συγκεκριμένες εμφανίσεις',
  `Performance_Performance_id` INT NOT NULL,
  `Date` DATETIME NOT NULL COMMENT 'ισως να μην χρειαζεται, επειδη μπορουμε να το βρουμε απο το event, αλλα το βλεπουμε λολ',
  PRIMARY KEY (`Staff_Staff_id`, `Performance_Performance_id`),
  INDEX `fk_Staff_has_Performance_Performance1_idx` (`Performance_Performance_id` ASC) VISIBLE,
  INDEX `fk_Staff_has_Performance_Staff1_idx` (`Staff_Staff_id` ASC) VISIBLE,
  CONSTRAINT `fk_Staff_has_Performance_Staff1`
    FOREIGN KEY (`Staff_Staff_id`)
    REFERENCES `mydb`.`Staff` (`Staff_id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Staff_has_Performance_Performance1`
    FOREIGN KEY (`Performance_Performance_id`)
    REFERENCES `mydb`.`Performance` (`Performance_id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
...
CREATE TABLE IF NOT EXISTS `mydb`.`Buyer` (
  `BuyerVisitor_id` INT NOT NULL,
  `Visitor_Visitor_id` INT NOT NULL,
  PRIMARY KEY (`BuyerVisitor_id`),
  INDEX `fk_Buyer_Visitor1_idx` (`Visitor_Visitor_id` ASC) VISIBLE,
  CONSTRAINT `fk_Buyer_Visitor1`
    FOREIGN KEY (`Visitor_Visitor_id`)
    REFERENCES `mydb`.`Visitor` (`Visitor_id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
...
-- -----------------------------------------------------
-- Table `mydb`.`Buyer_has_Ticket`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`Buyer_has_Ticket` ;

CREATE TABLE IF NOT EXISTS `mydb`.`Buyer_has_Ticket` (
  `Buyer_BuyerVisitor_id` INT NOT NULL,
  `Ticket_Ticket_id` INT NOT NULL,
  `datetime` DATETIME NOT NULL,
  PRIMARY KEY (`Buyer_BuyerVisitor_id`, `Ticket_Ticket_id`),
  INDEX `fk_Buyer_has_Ticket_Ticket1_idx` (`Ticket_Ticket_id` ASC) VISIBLE,
  INDEX `fk_Buyer_has_Ticket_Buyer1_idx` (`Buyer_BuyerVisitor_id` ASC) VISIBLE,
  CONSTRAINT `fk_Buyer_has_Ticket_Buyer1`
    FOREIGN KEY (`Buyer_BuyerVisitor_id`)
    REFERENCES `mydb`.`Buyer` (`BuyerVisitor_id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Buyer_has_Ticket_Ticket1`
    FOREIGN KEY (`Ticket_Ticket_id`)
    REFERENCES `mydb`.`Ticket` (`Ticket_id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
